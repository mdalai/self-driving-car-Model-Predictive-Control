    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
           ...
           ***         Polynomial fit                         ***
           *******************************************************/
          // transform waypoints double vector to Eigen::vector
          double* ptrx = &ptsx[0];
          Eigen::Map<Eigen::VectorXd> ptsx_transform(ptrx, 6);   // 6 elements

          double* ptry = &ptsy[0];
          Eigen::Map<Eigen::VectorXd> ptsy_transform(ptry, 6);

          // Fitting a polynomial to given waypoints
          auto coeffs = polyfit(ptsx_transform, ptsy_transform, 3);  // 3 IS polynomial degree/order
          ...
          double cte = polyeval(coeffs, 0);  // x is car's position in car coordinate which is (0,0)
