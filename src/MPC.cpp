...
size_t N = 10;    // 5    10   50    100  5   50
double dt = 0.1;  // 0.2  0.1  0.02  0.01 1  0.1
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2] * x0 * x0 + coeffs[3] * x0 * x0 * x0; //polynomial value
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  size_t n_vars = 6 * N + 2 * (N-1);  //state 6 elements, actuators 2 and N=10: 6*10+2*9
  // TODO: Set the number of constraints
  size_t n_constraints = 6 * N; // state 6 elements, N 10 --> contraints
  ...
  /////////// Upper and Lower limits for the constraints /////
